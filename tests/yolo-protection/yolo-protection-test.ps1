# YOLO Mode Protection Hook (PowerShell) - Version de test
...
# D'abord vérifier la variable d'environnement pour les tests
if ($env:CLAUDE_YOLO_MODE -eq "true") {
    $YoloMode = $true
    Write-Host "🔧 YOLO Mode détecté via variable d'environnement" -ForegroundColor Magenta
}

# Ensuite vérifier les processus Claude
try {
    ...
    foreach ($Process in $ClaudeProcesses) {
        ...
        if ($CommandLine -match "--dangerously-skip-permissions") {
            $YoloMode = $true
            Write-Host "🔧 YOLO Mode détecté via processus Claude" -ForegroundColor Magenta
            break
        }
    }
} catch {
    Write-Host "⚠️  Erreur lors de la vérification des processus: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
if ($FoundDangerous) {
    ...
    Write-Host "⚠️  Mode automatique - Simulation de blocage (pas d'input interactif)" -ForegroundColor Yellow
